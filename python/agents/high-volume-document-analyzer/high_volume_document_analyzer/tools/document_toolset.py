...
def get_model_instance() -> GenerativeModel:
    ...
    if _MODEL_INSTANCE is None:
        try:
            ...
            if not project_id:
                logging.warning("Project ID could not be determined. Ensure GOOGLE_CLOUD_PROJECT is set.")
...
async def analyze_document_next_chunk(
    ...
    try:
        ...
        if reset_search:
            logging.info(f"TOOL: Reset requested. Restarting index to 0 (Order: {sort_order}).")
        ...
        if not all_urls:
            logging.info(f"TOOL: Fetching URLs for collection {collection_id}...")
        ...
        start_label = f"#{total_docs - current_idx}" if is_descending else f"#{current_idx + 1}"
        end_label = f"#{total_docs - end_idx + 1}" if is_descending else f"#{end_idx}"
        
        logging.info(f"TOOL: Batch {current_idx+1}-{end_idx}. Real IDs: {start_label} to {end_label}")
        ...
        if not downloaded_files:
            ...
            return {"status": "continue", "content": "Error downloading. Trying next...", "progress": f"{end_idx}/{total_docs}"}
        ...
        for i, file_data in enumerate(downloaded_files):
            ...
            real_doc_num = (total_docs - (current_idx + i)) if is_descending else (current_idx + i + 1)
            label = f"Document #{real_doc_num}"
            prompt_parts.append(f"\n=== {label} ===\n")
            
            try:
                if file_data.get('is_binary'):
                    prompt_parts.append(Part.from_data(data=file_data['data'], mime_type=file_data['mime_type']))
                else:
        ...
        if valid_docs_count == 0:
            return {"status": "continue", "content": "No valid documents. Continuing...", "progress": f"{end_idx}/{total_docs}"}
